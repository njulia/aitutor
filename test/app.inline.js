        ...
        let currentTraceId = null; // Last AI trace, used for optional thumbs feedback
        ...
        // Capture the trace ID returned by the FastAPI Langfuse middleware.
        // The secret Langfuse key always stays on the server.
        const homeworkMagicFetch = window.fetch.bind(window);
        window.fetch = async function(...args) {
            const response = await homeworkMagicFetch(...args);
            const target = typeof args[0] === 'string' ? args[0] : (args[0]?.url || '');
            if (!target.includes('/api/feedback')) {
                const traceId = response.headers.get('X-Langfuse-Trace-Id');
                if (traceId) currentTraceId = traceId;
            }
            return response;
        };

        function traceFeedbackButtons() {
            if (!currentTraceId) return '';
            return `
                <div class="trace-feedback" style="margin-top:16px; padding-top:12px; border-top:1px solid #d7e8d8;">
                    <span style="font-weight:600; margin-right:8px;">Was this helpful?</span>
                    <button class="btn btn-secondary" style="padding:7px 14px;" onclick="sendTraceFeedback(1, this)">👍 Yes</button>
                    <button class="btn btn-secondary" style="padding:7px 14px;" onclick="sendTraceFeedback(0, this)">👎 Not really</button>
                    <span class="trace-feedback-status" aria-live="polite" style="margin-left:8px;"></span>
                </div>`;
        }

        async function sendTraceFeedback(value, button) {
            const traceId = currentTraceId;
            if (!traceId) return;
            const container = button.closest('.trace-feedback');
            const status = container?.querySelector('.trace-feedback-status');
            ...
            try {
                ...
                if (status) status.textContent = data.success ? 'Thank you!' : 'Feedback could not be saved.';
            } catch (error) {
                console.error('Feedback error:', error);
                if (status) status.textContent = 'Feedback could not be saved.';
            ...
            try {
                const response = await fetch('/api/review', {
                    ...
                    body: JSON.stringify({
                        ...
                        homework_doc_id: hw.doc_id // Pass doc_id for potential use in review
                    })
                });
