...
required_price_variables=(
  ...
  STRIPE_PRICE_SCHOOL_HOMEWORK_MONTHLY
)
...
if [[ "${STRIPE_PRICE_TRIAL_5DAY}" == "${STRIPE_PRICE_HOMEWORK_MONTHLY}" \
   ...
   || "${STRIPE_PRICE_HOMEWORK_MONTHLY}" == "${STRIPE_PRICE_ELEVENPLUS_MONTHLY}" \
   || "${STRIPE_PRICE_HOMEWORK_MONTHLY}" == "${STRIPE_PRICE_SCHOOL_HOMEWORK_MONTHLY}" \
   || "${STRIPE_PRICE_ELEVENPLUS_MONTHLY}" == "${STRIPE_PRICE_SCHOOL_HOMEWORK_MONTHLY}" \
   || "${STRIPE_PRICE_TRIAL_5DAY}" == "${STRIPE_PRICE_SCHOOL_HOMEWORK_MONTHLY}" ]]; then
...
gcloud run services update "${GCP_SERVICE}" \
  --project "${GCP_PROJECT_ID}" \
  --region "${GCP_REGION}" \
  --update-env-vars "STRIPE_BILLING_ENABLED=true,STRIPE_EXPECTED_LIVEMODE=true,STRIPE_PRICING_TABLE_ID=${STRIPE_PRICING_TABLE_ID},STRIPE_PUBLISHABLE_KEY=${STRIPE_PUBLISHABLE_KEY},STRIPE_PRICE_TRIAL_5DAY=${STRIPE_PRICE_TRIAL_5DAY},STRIPE_PRICE_HOMEWORK_MONTHLY=${STRIPE_PRICE_HOMEWORK_MONTHLY},STRIPE_PRICE_ELEVENPLUS_MONTHLY=${STRIPE_PRICE_ELEVENPLUS_MONTHLY},STRIPE_PRICE_SCHOOL_HOMEWORK_MONTHLY=${STRIPE_PRICE_SCHOOL_HOMEWORK_MONTHLY}" \
