...
"""
11+ Verbal Reasoning 55-Set Topic-Mastery Plan Generator
=========================================================

Generates a comprehensive 55-Set Topic-Mastery Verbal Reasoning curriculum (11 key topics, 
...
Saves the generated plan to:
  - 11_Plus_VR_55_Set_Topic_Mastery_Plan.json
  - 11_Plus_VR_55_Set_Topic_Mastery_Plan.md
...
try:
    from scripts.elevenplus.elevenplus_vr_generator import generate_11plus_vr_homework
...
MAP_TOPIC = {
    "Letter Series: Simple & Alternating Steps": "Letter Series",
    "Number Series: Arithmetic & Geometric": "Number Series",
    "Word Analogies: Opposite & Category Pairs": "Word Analogies (Related Pairs)",
    "Closest in Meaning: Synonym Matching": "Closest in Meaning (Synonyms)",
    "Opposites: Antonym Identification": "Opposites (Antonyms)",
    "Compound Words: Dual Bridge Building": "Compound Words",
    "Hidden Words: Substring Junctions": "Hidden Words",
    "Letter Codes: Alphabetical Ciphers": "Letter Codes",
    "Odd One Out: Category Classification": "Odd One Out",
    "Insert a Letter: Double Word Completion": "Insert a Letter (Completes Both Words)",
    "Logical Sequences & Alternating Codes": "Letter Codes"
}
...
def generate_topic_mastery_plan() -> list:
    """Generate all 55 sets of the Verbal Reasoning Topic-Mastery Plan."""
    print("Generating the 55-Set Verbal Reasoning Topic-Mastery Curriculum...")
    
    if not generate_11plus_vr_homework:
        print("Error: Could not import generators from elevenplus_vr_generator.py!")
        sys.exit(1)
    ...
    for topic_idx, topic in enumerate(TOPIC_LIST, start=1):
        ...
        base_topic = MAP_TOPIC[topic]
        
        for set_num in range(1, 6):
            tier_info = MASTERY_TIERS[set_num]
            
            # We use a deterministic but randomized seed/index offset so each set has unique questions
            seed_index = topic_idx * 100 + set_num
            
            # Generate the 10 MCQ questions for this set
            raw_content, answer_records = generate_11plus_vr_homework(base_topic, seed_index)
            
            # Modify and decorate the content to highlight the mastery level
            header = (
                f"========================================================================\n"
                f"11+ VERBAL REASONING TOPIC MASTERY PLAN - SET {global_set_index} of 55\n"
                f"Topic: {topic}\n"
            ...
            skip_header = True
            for line in lines:
                if skip_header and (line.startswith("11+ Verbal Reasoning Practice") or line.startswith("Answer each question")):
                    continue
            ...
            doc_id = f"elevenplus_vr_mastery_set_{global_set_index:03d}"
...
def save_to_json(all_sets: list, filepath: str):
    ...
    print(f"\n[Success] Saved Verbal Reasoning Topic-Mastery JSON data to: {filepath}")

def save_to_markdown(all_sets: list, filepath: str):
    """Save the full curriculum to an elegant Markdown file."""
    md_lines = []
    md_lines.append("# 11+ Verbal Reasoning 55-Set Topic-Mastery Curriculum & Practice Sets\n")
    md_lines.append("Formulated for Henrietta Barnett, Tiffin, CSSE, and St Olave's Verbal Reasoning papers.\n")
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
    print(f"[Success] Saved Verbal Reasoning Topic-Mastery Markdown to: {filepath}")

def main():
    print("==========================================================")
    print(" 11+ Verbal Reasoning 55-Set Topic-Mastery Curriculum Gen ")
    print("==========================================================\n")
    
    all_sets = generate_topic_mastery_plan()
    
    # Save files to workspace root for easy download and verification
    save_to_json(all_sets, "11_Plus_VR_55_Set_Topic_Mastery_Plan.json")
    save_to_markdown(all_sets, "11_Plus_VR_55_Set_Topic_Mastery_Plan.md")
