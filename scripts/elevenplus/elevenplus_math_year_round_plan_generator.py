...
def get_questions_for_week(week_num: int) -> list:
    ...
    if week_num == 1:
        ...
        questions.append({
            ...
            "options": ["Add 200, then add 1", "Add 100, then add 99", "Add 200, then subtract 1", "Add 200, then subtract 2", "Multiply by 2, then subtract 1"],
            "correctLetter": "C",
        ...
        questions.append({
            ...
            "options": ["(4 + 6) × 3 - 2", "4 + (6 × 3) - 2", "4 + 6 × (3 - 2)", "(4 + 6 × 3) - 2", "Brackets are not needed"],
            "correctLetter": "A",
...
def generate_markdown_plan() -> str:
    ...
    md.append("*Congratulations on working through this plan! Regular practice, diligent step-by-step working, and double-checking calculations are the keys to securing a high-accuracy selective school score.*")
    return "\n".join(md)
