...
"""
11+ English 55-Set Topic-Mastery Plan Generator
==============================================

Generates a comprehensive 55-Set Topic-Mastery English curriculum (11 key topics, 
...
Saves the generated plan to:
  - 11_Plus_English_55_Set_Topic_Mastery_Plan.json
  - 11_Plus_English_55_Set_Topic_Mastery_Plan.md
...
try:
    from scripts.elevenplus.elevenplus_english_generator import generate_11plus_english_homework
...
MAP_TOPIC = {
    "Spelling: Vowels & Double Letters": "Spelling",
    "Spelling: Prefixes, Suffixes & Silent Letters": "Spelling",
    "Punctuation: Capital Letters & Full Stops": "Capital Letters and Punctuation",
    "Punctuation: Apostrophes, Commas & Speech": "Capital Letters and Punctuation",
    "Vocabulary: Contextual Word Choice (Cloze)": "Vocabulary: Word Choice (Cloze)",
    "Vocabulary: Synonyms & Definitions": "Vocabulary: Synonyms and Antonyms",
    "Vocabulary: Antonyms & Word Opposites": "Vocabulary: Synonyms and Antonyms",
    "Grammar: Subject-Verb Agreement & Tenses": "Grammar",
    "Grammar: Pronouns, Prepositions & Conjunctions": "Grammar",
    "Comprehension: Fact Retrieval & Location": "Reading Comprehension",
    "Comprehension: Inference, Meaning & Tone": "Reading Comprehension"
}
...
def generate_topic_mastery_plan() -> list:
    """Generate all 55 sets of the English Topic-Mastery Plan."""
    print("Generating the 55-Set English Topic-Mastery Curriculum...")
    
    if not generate_11plus_english_homework:
        print("Error: Could not import generators from elevenplus_english_generator.py!")
        sys.exit(1)
    ...
    for topic_idx, topic in enumerate(TOPIC_LIST, start=1):
        ...
        for set_num in range(1, 6):
            ...
            header = (
                f"========================================================================\n"
                f"11+ ENGLISH TOPIC MASTERY PLAN - SET {global_set_index} of 55\n"
                f"Topic: {topic}\n"
            ...
            doc_id = f"elevenplus_english_mastery_set_{global_set_index:03d}"
...
def save_to_json(all_sets: list, filepath: str):
    ...
    print(f"\n[Success] Saved English Topic-Mastery JSON data to: {filepath}")

def save_to_markdown(all_sets: list, filepath: str):
    """Save the full curriculum to an elegant Markdown file."""
    md_lines = []
    md_lines.append("# 11+ English 55-Set Topic-Mastery Curriculum & Practice Sets\n")
    md_lines.append("Formulated for Henrietta Barnett, Tiffin, CSSE, and St Olave's English entrance papers.\n")
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
    print(f"[Success] Saved English Topic-Mastery Markdown to: {filepath}")

def main():
    print("==========================================================")
    print("   11+ English 55-Set Topic-Mastery Curriculum Generator   ")
    print("==========================================================\n")
    
    all_sets = generate_topic_mastery_plan()
    
    # Save files to workspace root for easy download and verification
    save_to_json(all_sets, "11_Plus_English_55_Set_Topic_Mastery_Plan.json")
    save_to_markdown(all_sets, "11_Plus_English_55_Set_Topic_Mastery_Plan.md")
    
    # Expose them to RAG if available
    if get_elevenplus_rag_store:
        try:
            print("\nRegistering English Topic-Mastery sets with the RAG Store...")
