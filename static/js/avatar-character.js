...
(function initialiseHomeworkMagicAvatar(global) {
  ...
  const AvatarData = global.HomeworkMagicAvatarData || {};
  const PROFILE_DEFAULTS = AvatarData.PROFILE_DEFAULTS;
  const CHARACTER_PRESETS = AvatarData.CHARACTER_PRESETS;
  const OPTIONS = AvatarData.OPTIONS;
  const ATTRIBUTES = AvatarData.ATTRIBUTES;
  const XP_STAGES = AvatarData.XP_STAGES;
  const AGE_SCALES = AvatarData.AGE_SCALES;
  const XP_SCALES = AvatarData.XP_SCALES;
  const GIRL_PROPORTION_TRANSFORMS = AvatarData.GIRL_PROPORTION_TRANSFORMS;
  const SKIN_PALETTES = AvatarData.SKIN_PALETTES;
  const HAIR_PALETTES = AvatarData.HAIR_PALETTES;
  const EYE_PALETTES = AvatarData.EYE_PALETTES;
  const OUTFIT_PALETTES = AvatarData.OUTFIT_PALETTES;
  const TROUSER_PALETTES = AvatarData.TROUSER_PALETTES;
  const BOTTOM_PALETTES = AvatarData.BOTTOM_PALETTES;
  const SHOE_PALETTES = AvatarData.SHOE_PALETTES;
